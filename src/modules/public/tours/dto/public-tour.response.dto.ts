...
import { TourAmenityDto } from '../../../admin/amenities/dto/amenity.response.dto';

export class PublicTourListResponseDto extends PageDto<PublicTourDto> {
	constructor(data: PublicTourDto[], meta: PageMetaDto) {
		super(data, meta);
	}
}

export class PublicTourDto extends BaseDto {
	@StringField({
...
export class PublicHotspotDto extends BaseDto {
...
export class PublicSceneDto extends BaseDto {
	@StringField({
	...
	@ApiProperty({
		...
		type: [PublicHotspotDto],
		required: false,
	})
	readonly hotspots?: PublicHotspotDto[];
	...
	constructor(data: Scene & { hotspots?: Hotspot[] }) {
		...
		if (data.hotspots) {
			this.hotspots = data.hotspots.map(
				(hotspot) => new PublicHotspotDto(hotspot),
			);
		}
	}
}

export class PublicTourDetailDto extends PublicTourDto {
	@ApiProperty({
		description: 'Tour scenes',
		type: [PublicSceneDto],
		required: false,
	})
	readonly scenes?: PublicSceneDto[];
	...
	constructor(
		...
		if (data.scenes) {
			this.scenes = data.scenes.map((scene) => new PublicSceneDto(scene));
		}
	}
}
